...
void * mymalloc(int x){
	...
	int sizeofData = sizeof(*(short*)(blockSim));
	...
	if(x > sizeof(blockSim) - sizeofMagic - sizeofData - 1 || x < 1){
	...
	//detects the magic block, intiialises if not found
		...
		if(x <= sizeof(blockSim) - sizeofMagic - sizeofData - 1 && x >= sizeof(blockSim) - sizeofMagic - (2*sizeofData) - 2){
		...
		//otherwise assumes there is space for reasonable metadata afterwards
			...
			*(short*)(blockSim+location) = sizeof(blockSim) - sizeofMagic - 2 - 2*(sizeofData) - x;
	...
	//runs if it is not the first time running malloc
		...
		while(flag == 0){
			...
			if(blockSim[location] == 1){		
				...
				location += *(short*)(blockSim + location) + sizeofData;					//reads the actual size value of the block, skips past it
				
				//if the location goes out of bounds
				if(location >= sizeof(blockSim) - 1 - sizeofData){
			...
			//if the in-use byte is 0, code will check to see if it is the right size, and if so, assign sizes and return the address to the user
				...
				if(x <= *(short*)(blockSim+(location+1)) && x + sizeofData + 1 >= *(short*)(blockSim+(location+1))){
				...
				//if the user requests space and the first block has more than enough; gives user space and splits the remaining space into metadata
				else if(*(short*)(blockSim+(location+1)) > (x + 1 + sizeofData)){
					...
					*(short*)(blockSim+location) = size - x - sizeofData - 1;
			...
			else if(x > *(short*)(blockSim+(location+1) + 1 + sizeofData)){
				//printf("\n	Free location, but not enough space.\n");
				
				location += *(short*)(blockSim + (location+1)) + 1 + sizeofData;					//reads the actual size value of the block, skips past it
...
void myfree(void * x){
	...
	if(x < &(blockSim[location + sizeofMagic + 1 + sizeofData]) || x >= &(blockSim[sizeof(blockSim)-1-sizeofData])){
	...
	//iterate down the array and check if the address given is the start of a block
	else if(x >= &(blockSim[0 + sizeofMagic + 1 + sizeofData]) || x < &(blockSim[sizeof(blockSim)-1-sizeofData])){
		int flag = 0;
		...
		while(flag == 0){
			...
			if(blockSim[location] == 0 && x == &(blockSim[location + 1 + sizeofData])){
			...
			else if(x > &(blockSim[location + 1 + sizeofData])){
				//printf("	Iterating the array.\n");
				
				prevLoc = location;
				
				location += 1;
				location += *(short*)(blockSim + location) + sizeofData;
			...
			//if the location's address is in-use and exactly what the user asked for, frees and starts looking through for adjacent free blocks
			else if(blockSim[location] == 1 && x == &(blockSim[location + 1 + sizeofData])){
				...
				if(blockSim[prevLoc] == 0 && location != prevLoc){
					...
					*(short*)(blockSim+prevLoc) += *(short*)(blockSim+(location+1)) + sizeofData + 1;
				...
				//OR is out of bounds
				if(blockSim[location] == 1 || location >= sizeof(blockSim) - 1 - sizeofData){
